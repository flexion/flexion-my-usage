...
- A `*.fixtures.*` file being excluded from coverage is not by itself proof it's test support - only that the gate skips it. `yarn lint` runs `scripts/check-fixtures-guard.mjs` (pure logic in `scripts/fixtures-guard.ts`) to check that separately: it fails if anything outside test code or test support imports a `*.fixtures.*` file, and it fails a `*.fixtures.*` file that can't be verified as test-only - it must either import `"vitest"` itself, or be referenced by a real `*.test.*` file (a static import, or the `new URL("./x.fixtures.ts", import.meta.url)` idiom a subprocess fixture uses to hand itself its own path).
